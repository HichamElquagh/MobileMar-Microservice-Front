...
const ProductDisplay = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (paymentMethod) {
      // Handle payment based on selected method (Stripe or PayPal)
      if (paymentMethod === 'stripe') {
        ...
        try {
          ...
          console.log(response.data.payment); // Log the response data
        ...
        const response = await axios.post(
          "http://localhost:3000/Payments/Paypal/1"
        )
        console.log(response.data.links); // Log the response data
        // Redirect the user to the PayPal payment page
        window.location.href = response.data.links.find((link: { rel: string; }) => link.rel === 'approve').href;        
      }
    }
  };
